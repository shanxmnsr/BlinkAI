...
export default function ChatContainer() {
  ...
  return (
    <div className="flex h-screen bg-slate-100 text-slate-800">
      <div className="flex flex-col w-full max-w-4xl mx-auto px-4 sm:px-6 py-4">

        {/* ================= EMPTY STATE ================= */}
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center flex-1 text-center">
            <h1 className="text-3xl sm:text-5xl font-semibold tracking-tight text-slate-900 mb-4">
              BlinkAI
            </h1>
            ...
            <div className="w-full max-w-xl flex gap-2 sm:gap-3 items-center bg-white p-2 sm:p-3 rounded-2xl shadow-md border border-slate-200">
              <input
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="Type a message..."
                ...
                className="flex-1 bg-transparent outline-none text-slate-700 placeholder:text-slate-400 text-sm sm:text-base"
              />
              <button
                onClick={handleSend}
                disabled={loading}
                className="px-4 sm:px-5 py-2 rounded-xl bg-slate-900 text-white text-sm font-medium hover:bg-slate-800 active:scale-95 transition-all"
              >
                {loading ? "Sending..." : "Send"}
              </button>
            </div>
          ...
          <>
            {/* ================= CHAT MESSAGES ================= */}
            <div className="flex-1 overflow-y-auto rounded-2xl bg-slate-50 shadow-md border border-slate-200 p-4 sm:p-6 space-y-3">
              {messages.map((msg, index) => (
                <MessageBubble key={index} role={msg.role} content={msg.content} />
              ))}
            ...
            {/* ================= INPUT ================= */}
            <div className="mt-3 sticky bottom-0 bg-white p-2 sm:p-3 rounded-2xl shadow-md border border-slate-200 flex gap-2 sm:gap-3 items-center">
              <input
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="Type a message..."
